...
function fibonacci(n) {
    let fib_seq = [0, 1];
    while (fib_seq[fib_seq.length - 1] < n) {
        let nextFib = fib_seq[fib_seq.length - 1] + fib_seq[fib_seq.length - 2];
        fib_seq.push(nextFib);
    }
    
    if (fib_seq.includes(n)) {
...
const jsonFaturamentos = JSON.stringify(faturamentos)

console.log(jsonFaturamentos)
...
let totalFaturamento = Object.values(faturamento).reduce((a, b) => a + b, 0);
...
console.log(inverterString(string));  // Resultado: "gnirts ed olpmeX"
